...
function TodayWeather() {
    ...
    const { weatherData, astronomyData } = useSelector((store) => store.data)

    console.log("today weather", weatherData)

    console.log("today astro",astronomyData)
    ...
    // const { moonrise, moonset, moonsign, phase_name, age, illumination, distance, diameter, tilt, next_full, next_moonrise, next_moonset, next_new } = astronomyData?.days[1] || {}
    // const { last_updated, temp_c, wind_dir, wind_degree, wind_kph, condition, air_quality, cloud, vis_km, uv, humidity, feelslike_c } = weatherData?.current || {}
    ...
    return (
        <main className='h-full'>
            ...
            {/* {weatherData?.forecast && weatherData?.current && astronomyData && (
                <div className='grid gap-y-5 '>
                    <div className='text-white py-10 min-w-full flex flex-col justify-between items-center bg-[url(https://media.istockphoto.com/id/825778252/tr/foto%C4%9Fraf/mavi-g%C3%B6ky%C3%BCz%C3%BC-ve-bulutlar-beyaz-arka-plan.jpg?s=612x612&w=0&k=20&c=Ej5iKzuVujUEbYZ_a8rM4koHLoAWeKGg_6EcuPGhd6E=)] bg-cover '>
                        ...
                        <div className='flex'>
                            <img width={150} src={condition?.icon} alt={condition?.text} />
                            <h2 className='font-bold text-5xl'>{temp_c}°C</h2>
                        </div>
                        ...
                        <div className='flex flex-1 items-center space-x-2 mt-1'>
                            <div style={{ transform: `rotate(${wind_degree + 90}deg)` }}>
                                <Icon name='arrow' size='24' />
                            </div>
                            <div>{wind_dir}</div>
                            <div>{wind_kph} km/h</div>
                        </div>
                        <div className=''>Last update: {moment(last_updated).format('HH:MM')}</div>
                    ...
                    <div className='container mx-auto grid md:grid-cols-2 bg-white p-10 rounded-xl'>
                        ...
                        <div className="flex items-center space-x-2 border-b md:mx-4 drop-shadow-lg p-2">
                            ...
                            <h2>Maximum Temperature: {weatherData.forecast.forecastday[1].day.maxtemp_c}°C</h2>
                        </div>
                        <div className="flex items-center space-x-2 border-b md:mx-4 drop-shadow-lg p-2">
                            <Icon name="minTemp" size="40" />
                            <h2>Minimum Temperature: {weatherData.forecast.forecastday[1].day.mintemp_c}°C</h2>
                        </div>
                        <div className="flex items-center space-x-2 border-b md:mx-4 drop-shadow-lg p-2">
                            <Icon name="sensedTemp" size="40" />
                            <h2>Sensed Temperature: {feelslike_c}°C</h2>
                        </div>
                        <div className="flex items-center space-x-2 border-b md:mx-4 drop-shadow-lg p-2">
                            <Icon name="autoTemp" size="40" />
                            <h2>Average Temperature: {weatherData.forecast.forecastday[1].day.avgtemp_c}°C</h2>
                        </div>
                        <div className="flex items-center space-x-2 border-b md:mx-4 drop-shadow-lg p-2">
                            <Icon name="changeRain" size="40" />
                            <h2>Chance of rain: %{weatherData.forecast.forecastday[1].day.daily_chance_of_rain}</h2>
                        </div>
                        <div className="flex items-center space-x-2 border-b md:mx-4 drop-shadow-lg p-2">
                            <Icon name="changeSnow" size="40" />
                            <h2>Chance of snow: %{weatherData.forecast.forecastday[1].day.daily_chance_of_snow}</h2>
                        </div>
                        <div className="flex items-center space-x-2 border-b md:mx-4 drop-shadow-lg p-2">
                            <Icon name="totalSnow" size="40" />
                            <h2>Total snow: {weatherData.forecast.forecastday[1].day.totalsnow_cm}cm</h2>
                        </div>
                    </div>
                        ...
                        <div className='container py-10 mx-auto grid lg:grid-flow-col place-content-center place-items-center bg-white md:p-10 rounded-xl'>
                            <div className="flex flex-col items-center font-semibold text-xl">
                                ...
                                <div className="flex items-center space-x-2 border-b md:mx-4 drop-shadow-lg p-2">
                                    ...
                                    <h2>Moon Rise: {moment(moonrise).format('HH:mm')} PM</h2>
                                </div>
                                <div className="flex items-center space-x-2 border-b md:mx-4 drop-shadow-lg p-2">
                                    <Icon name="moonSet" size="40" />
                                    <h2>Moon Set: {moment(moonset).format('hh:mm')} AM</h2>
                                </div>
                            </div>
                            <div>
                                <p className='text-lg leading-8 p-10'>As we examine the information regarding the state of the Moon, it is quite intriguing. As of today,
                                    the Moon's celestial position is rather fascinating. The Moon will rise at {moment(moonset).format('hh:mm')} AM and set at {moment(moonrise).format('hh:mm')} PM.
                                    The Moon is identified to be in the {moonsign} zodiac sign, and its current phase is {phase_name} with an age of {age} days.
                                    The illumination of the Moon in the sky is {illumination} percent.
                                    The Moon is situated at a distance of {distance} kilometers from Earth with a diameter of {diameter} units.
                                    Its tilt is at {tilt} degrees. The next full moon is expected to occur on {moment(next_full).format('MMMM DD YYYY hh:mm')}  PM. In the coming days,
                                    the Moon's rise and set times will change; it will rise at {moment(next_moonrise).format('hh:mm')} PM tomorrow and set at {moment(next_moonset).format('hh:mm')} AM the next day.
                                    The Moon's next new phase will occur on {moment(next_new).format('MMMM DD YYYY hh:mm')} PM. Evaluating all these pieces of information,
                    ...
                    <div className='bg-white flex-col p-10 container mx-auto rounded-xl grid md:grid-cols-2'>
                        <div className='flex items-center text-lg justify-center max-md:py-10'>
                            <div>{airQuality(air_quality['us-epa-index'])}</div>
                        </div>
                        <div>
                            <div className="flex items-center space-x-2 border-b md:mx-4 drop-shadow-lg p-2">
                                <h3 className='flex w-full'><strong className='flex-1'>Nitrogen dioxide NO2:</strong> <span>{air_quality.no2}</span></h3>
                            </div>
                            <div className="flex items-center space-x-2 border-b md:mx-4 drop-shadow-lg p-2">
                                <h3 className='flex w-full'><strong className='flex-1'>gb-defra-index :</strong> {air_quality['gb-defra-index']}</h3>
                            </div>
                            <div className="flex items-center space-x-2 border-b md:mx-4 drop-shadow-lg p-2">
                                <h3 className='flex w-full'><strong className='flex-1'>Carbon monoxide CO:</strong> {air_quality.co}</h3>
                            </div>
                            <div className="flex items-center space-x-2 border-b md:mx-4 drop-shadow-lg p-2">
                                <h3 className='flex w-full'><strong className='flex-1'>pm10 :</strong> {air_quality.pm10}</h3>
                            </div>
                            <div className="flex items-center space-x-2 border-b md:mx-4 drop-shadow-lg p-2">
                                <h3 className='flex w-full'><strong className='flex-1'>pm2_5 :</strong> {air_quality.pm2_5}</h3>
                            </div>
                            <div className="flex items-center space-x-2 border-b md:mx-4 drop-shadow-lg p-2">
                                <h3 className='flex w-full'><strong className='flex-1'>Ozone O3:</strong> {air_quality.o3}</h3>
                            </div>
                            <div className="flex items-center space-x-2 border-b md:mx-4 drop-shadow-lg p-2">
                                <h3 className='flex w-full'><strong className='flex-1'>Sulphur dioxide SO2:</strong> {air_quality.so2}</h3>
                            </div>
                            <div className="flex items-center space-x-2 border-b md:mx-4 drop-shadow-lg p-2">
                                <h3 className='flex w-full'><strong className='flex-1'>us-epa-index :</strong> {air_quality['us-epa-index']}</h3>
                            </div>
                        </div>
